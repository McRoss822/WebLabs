...
export const Content = () => {
  ...
  const [addedToCart, setAddedToCart] = useState(false); // State to control the confirmation message

  const handleAddToCart = () => {
    dispatch(addToCart({ id, name, price, count }));
    setAddedToCart(true); // Set addedToCart to true when item is added
    setTimeout(() => {
      setAddedToCart(false); // Hide confirmation message after a certain time (e.g., 3 seconds)
    }, 3000);
  };
        ...
        <ContentWrapper>
            <Heading>
                ...
                <Info>
                    ...
                    <Fields>
                        <FieldInput fieldType="Amount" count={count} setCount={setCount}/>
                        <FieldSelect/>
                    </Fields>
                </Info>
                </Heading>
...
`
const FieldInput = ({fieldType, count, setCount}) =>{
